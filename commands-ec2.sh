...
git pull && node ace build --production --ignore-ts-errors && docker build -t "march1st-backend" . && docker run -itd --name march1st-backend -p8080:8080 march1st-backend && docker logs --follow --tail 100 march1st-backend
